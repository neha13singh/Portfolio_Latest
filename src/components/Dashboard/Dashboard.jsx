                                ...
                                {portfolioData.education.map((edu, idx) => (
                                    <div key={idx} className="relative pl-4 border-l border-agent-border group hover:border-white transition-colors">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            {edu.score && <span className="text-white text-xs font-mono border border-white/30 px-1 rounded">{edu.score}</span>}
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <Card title="Skill_Matrix" delay={0.4}>
                        <div className="h-[270px] overflow-y-auto custom-scrollbar pr-2 flex flex-wrap gap-2 content-start">
                            {portfolioData.skills.map((skill, idx) => (
                                <span key={idx} className="px-3 py-1 bg-agent-header border border-agent-border text-gray-300 text-xs rounded hover:border-white hover:text-white transition-colors cursor-default">
                                    {skill}
                                </span>
                            ))}
                        </div>
                            ...
                            <ul className="space-y-3">
                                {portfolioData.achievements?.map((ach, idx) => (
                                    <li key={idx} className="flex items-start gap-2 text-xs group">
                                        ...
                                        <a href={ach.link} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white hover:underline decoration-white/50 transition-colors">
                                            {ach.title}
                                        </a>
                                    </li>
                                ))}
